...
A banking application based on the [project starter template](https://github.com/cybersecuritybase/cybersecuritybase-project). This app uses a SQL database (h2 database engine to exact). The database is set to in memory only. This means it will reset if you restart the app. There are users (customers) with bank accounts. Each user can have multiple bank accounts, and money can be transferred from a bank account to another. There’s also some admin only functionality. 
